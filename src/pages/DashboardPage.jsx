...
const DashboardPage = () => {
  ...
  const addButton = async (id) => {
    try {
      const formData = new FormData();
      formData.append("id_content", id);
  
      const response = await AddToWatchLater(formData);
      ...
      toast.error("An error occurred while adding to watch later");
    }
  };
  ...
  return (
    <Container className="mt-4">
      <Stack direction="horizontal" gap={3} className="mb-3">
        <h1 className="h4 fw-bold mb-0 text-nowrap textColorWhite">
          Rekomendasi Untukmu
        </h1>
